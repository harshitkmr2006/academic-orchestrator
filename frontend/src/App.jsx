...
try {
  const response = await fetch(`${API}/api/goals/${goal.id}`, {
    method: 'PUT',
    headers: {
      'Content-Type': 'application/json',
      Authorization: `Bearer ${token}`
...
try {
  const response = await fetch(`${API}/api/study-sessions/${id}`, {
    method: 'DELETE',
    headers: { Authorization: `Bearer ${token}` }
  });
  ...
  <main className="portal-workspace-main">
        ...
        <section className="stats-grid">
          <div className="card stat-card">
            ...
            <div className="stat-value">
              {totalHours} <span className="stat-value-sub">Hours</span>
            </div>
        ...
        <div className="task-workspace-grid">
          <div className="planner-forms-col">
            ...
            <div className="card glass-card">
              <h3 className="card-title" style={{ marginBottom: '1rem', paddingBottom: '0.4rem', borderBottom: '1px solid var(--border-color)' }}>
                Study Milestones
              </h3>
              ...
              <div style={{ marginTop: '1.5rem' }}>
                <GoalList goals={goals} onToggleGoal={handleToggleGoal} onDeleteGoal={handleDeleteGoal} />
              </div>
            </div>
          </div>
    ...
    {activeTab === 'tracker' && (
      <div className="workspace-tab-fade">
        ...
        <StudyTracker sessions={sessions} onSessionLogged={handleSessionLogged} onSessionDeleted={handleSessionDeleted} />
      </div>
    )}
