...
export default function AIEngine({ tasks }) {
  ...
  const fetchPlan = async (forceGenerate = false) => {
    ...
    try {
      const response = await fetch(`${API}/api/ai/generate-plan`, {
        method: 'POST',
        headers: {
  ...
  const parseMarkdown = (text) => {
    ...
    return text.split('\n').map((line, idx) => {
      if (line.startsWith('# ')) {
        return (
          <h2 key={idx} className="ai-plan-h1" style={{ fontSize: '1.5rem', color: 'var(--text-main)', marginTop: '1.25rem', marginBottom: '0.75rem' }}>
            {line.replace('# ', '')}
          </h2>
        );
      }
      ...
      if (line.startsWith('## ')) {
        return (
          <h3 key={idx} className="ai-plan-h2" style={{ fontSize: '1.25rem', color: 'var(--primary)', marginTop: '1.25rem', marginBottom: '0.5rem' }}>
            {line.replace('## ', '')}
          </h3>
        );
      }
      ...
      if (line.startsWith('### ')) {
        return (
          <h4 key={idx} className="ai-plan-h3" style={{ fontSize: '1.1rem', color: 'var(--text-main)', marginTop: '1rem', marginBottom: '0.5rem' }}>
            {line.replace('### ', '')}
          </h4>
        );
      }
      ...
      if (line.startsWith('* ') || line.startsWith('- ')) {
        ...
        return (
          <li key={idx} className="ai-plan-li" style={{ marginLeft: '1rem', listStyleType: 'disc', fontSize: '0.9rem', marginBottom: '0.35rem' }}>
      ...
      if (line.startsWith('> ')) {
        return (
          <blockquote key={idx} className="ai-plan-quote" style={{ borderLeft: '3px solid var(--primary)', paddingLeft: '0.75rem', color: 'var(--text-muted)', fontSize: '0.85rem', fontStyle: 'italic', margin: '0.75rem 0' }}>
            {line.replace('> ', '')}
          </blockquote>
        );
      }
      ...
      return (
        <p key={idx} className="ai-plan-p" style={{ fontSize: '0.9rem', lineHeight: '1.5', marginBottom: '0.5rem' }}>
          {renderBoldText(line)}
        </p>
      );
    });
  ...
  return (
    <div className="card glass-card ai-planner-card glowing-border">
      ...
      {error && (
        <div className="alert alert-danger" style={{ fontSize: '0.8rem', padding: '0.5rem' }}>
          {error}
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="empty-state" style={{ textAlign: 'center', padding: '2rem 0' }}>
            ...
            <p style={{ fontSize: '0.9rem', color: 'var(--text-muted)' }}>
              Generate a custom study plan customized to your deadlines, goals, and placement subjects.
            </p>
